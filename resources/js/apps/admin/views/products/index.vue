                        ...
                        <div class="row">
                            ...
                            <div class="table-responsive">
                                    ...
                                    <table role="table" aria-busy="false" aria-colcount="5" class="table table-striped table-bordered" id="__BVID__415">
                                        <thead role="rowgroup">
                                            <tr role="row">
                                                <th role="columnheader" scope="col" aria-colindex="0"  v-if="category.main_category !== ''"><div></div></th>
                                                <th role="columnheader" scope="col" aria-colindex="1"><div>Image</div></th>
                                                <th role="columnheader" scope="col" aria-colindex="2"><div>Name</div></th>
                                                <th role="columnheader" scope="col" aria-colindex="3"><div>Make</div></th>
                                                <th role="columnheader" scope="col" aria-colindex="4"><div>Model Number</div></th>
                                                <th role="columnheader" scope="col" aria-colindex="5"><div>Sku</div></th>
                                                <th role="columnheader" scope="col" aria-colindex="6"><div>Is Published</div></th>
                                                <th role="columnheader" scope="col" aria-colindex="7"><div>Created At</div></th>
                                                <th role="columnheader" scope="col" aria-colindex="8" aria-label="Action" class="text-center"><div>action</div></th>
                                            </tr>
                                        </thead>
                                        <tbody role="rowgroup">
                                            ...
                                            <tr v-for="the_data in displayedProducts"  :key="category.id" role="row" class="">

                                                <td aria-colindex="0" class="handle" v-if="category.main_category !== ''">
                                                    <div>
                                                ...
                                                <td aria-colindex="1" role="cell">
                                                    <img style="width: 80px;" :src="'/storage/' + the_data.main_image_path" class="rounded profile-img" alt="avatar" />
                                                </td>
                                                
                                                <td aria-colindex="1" role="cell">{{ the_data.name }}</td>
                                                <td aria-colindex="2" role="cell">{{ the_data.product_brand.name }}</td>
                                                <td aria-colindex="3" role="cell">{{ the_data.model_number }}</td>
                                                <td aria-colindex="4" role="cell">{{ the_data.sku }}</td>
                                                <td aria-colindex="4" role="cell">
                                                    
                                                    <span v-if="the_data.is_published === 1" class="badge badge-success inv-status">Published</span>

                                                    <span v-if="the_data.is_published !== 1" class="badge badge-danger inv-status">Not Published</span>

                                                </td>
                                                <td aria-colindex="5" role="cell">{{ the_data.created_at }}</td>


                                               

                                                <td aria-colindex="6" role="cell" class="text-center">
                                                     <router-link :to="{ name: 'products.edit', params: { id: the_data.id } }" class="badge bg-info" data-bs-toggle="tooltip" data-bs-placement="top">
                                                        ...
                                                        <a href="javascript:;" @click.prevent="deleteProduct(the_data.id)" class="ms-2 badge bg-danger" data-bs-toggle="tooltip" data-bs-placement="top">
                            ...
                            <nav aria-label="Page navigation" style="margin-top: 20px;">
                                <ul class="pagination justify-content-center">
                                  <li class="page-item" :class="{ disabled: currentPage === 1 }">
                                    <router-link class="page-link page-link-prev" :to="getCategoryLink(the_category.id, 1 )" aria-label="Previous" tabindex="-1" aria-disabled="true" @click="goToPreviousPage">
                                  ...
                                  <li class="page-item" v-for="page in generatePageLinks" :key="page" :class="{ active: page === currentPage }">

                                    <template v-if="isInteger(page)">
                                      <router-link class="page-link" :to="getCategoryLink(the_category.id, page )" @click="goToThisPage(page)">
                                        {{ page }}
                                      </router-link>
                                    </template>
                                 ...
                                 <!--  <li class="page-item page-item-total">of {{ totalPages }}</li> -->
                                  ...
                                  <li class="page-item" :class="{ disabled: currentPage === totalPages }">
                                    <router-link class="page-link page-link-next" :to="getCategoryLink(the_category.id, totalPages )" aria-label="Next" @click="goToNextPage">
                                      Last
                                      <span aria-hidden="true"><i class="icon-long-arrow-right"></i></span>
                                    </router-link>
                                  </li>
                                  ...
                                  <li class="page-item" :class="{ disabled: currentPage === totalPages }">
                                    <router-link class="page-link page-link-next" :to="getCategoryLink(the_category.id, currentPage + 1 )" aria-label="Next" @click="goToNextPage">
                                      Next
                                      <span aria-hidden="true"><i class="icon-long-arrow-right"></i></span>
                                    </router-link>
                                  </li>
...
<script setup>
        ...
        try {
            ...
            const response = await axios.get('/api/get-products-admin', {
                params: {
                    ...
                    filter_category_id: category.filter_category_id && category.filter_category_id.id || null,
                },
            });
  ...
  const updateDatabaseOrder = async (formData) => {
    ...
    try {
        ...
        const response = await axios.post('/api/products-update-order', { data: formData });
