<template>
  <div v-show="store.state.is_show_sidebar" class="bg-gray-800 text-white h-full mt-28 fixed top-0 left-2 mr-6 w-56" style="z-index: 1035" id="sidebar-parent">
    <nav id="sidebar" class="overflow-y-auto" style="height: 82vh !important; scrollbar-width: none; overflow-x: hidden; z-index: 1035;">
      <perfect-scrollbar class="menu-categories mb-4" tag="div">
        <div v-for="(item, index) in menuItems" :key="index" class="menu">
          <div @click="toggleSubmenu(item.id)" class="cursor-pointer flex items-center justify-between p-2 text-gray-300 hover:bg-gray-700 hover:text-white transition duration-150">
          ...
          <div v-show="isSubmenuOpen(item.id)" class="submenu pl-4 bg-gray-700">
            <div v-for="(subItem, subIndex) in item.subItems" :key="subIndex">
              <router-link :to="subItem.route" @click="toggleMobileMenu" class="block px-6 py-2 text-gray-300 hover:bg-gray-600 hover:text-white transition duration-150">
