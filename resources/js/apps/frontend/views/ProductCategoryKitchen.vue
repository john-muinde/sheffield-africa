      ...
      <nav aria-label="breadcrumb" class="breadcrumb-nav border-0 mb-0">
           <div class="container d-flex align-items-center">
               <ol class="breadcrumb">
                   ...
                   <li class="breadcrumb-item active" aria-current="page">{{ the_category.name }}</li>
               </ol>
           </div>
       </nav>
      <!-- End .page-header --> 
               ...
               <div class="col-lg-10">
                  <div class="toolbox">
                     <div class="toolbox-left">
                        <div class="toolbox-info">
                           Showing <span>{{ totalCountperPage }} of {{ totalProducts }}</span> Products 
                        </div>
                        <!-- End .toolbox-info -->
                     </div>
                     <!-- End .toolbox-left -->
                     <div class="toolbox-right">
                        <div class="toolbox-sort">
                           ...
                           <div class="select-custom">
                              <select name="sortby" id="sortby" class="form-control" v-model="selectedSortOption">
                  ...
                  <div class="products mb-3 products-section">
                     <div class="row">
                        ...
                        <div class="col-6 col-md-3 col-lg-2 col-xl-2" v-for="product in displayedProducts" :key="product.id">

                           <div class="product product-7 text-center">
                              <figure class="product-media">
                                 <!-- <span class="product-label label-new">New</span>  -->
                                 <router-link :to="getProductLink(product.id, product.name, product.model_number, the_category.name)">
                                    <img :src="'/storage/' + product.main_image_path" v-lazy:src="'/storage/' + product.main_image_path" alt="Product image" class="product-image"
                                       /> 
                                 </router-link>
                                 ...
                                 <div class="product-action">
                                    <button type="button" @click="addToCart(product)" class="btn-product btn-cart"
                              ...
                              <div class="product-body">
                                 <div class="product-cat">
                                    <router-link :to="getProductLink(product.id, product.name, product.model_number, the_category.name)" >{{ product.product_brand.name }}</router-link>
                                 </div>
                                 <!-- End .product-cat --> 
                                 <h3 class="product-title">
                                    <router-link :to="getProductLink(product.id, product.name, product.model_number, the_category.name)">{{ product.name }}</router-link>
                                 </h3>
                                 <!-- End .product-title -->
                  ...
                  <nav aria-label="Page navigation">
                    <ul class="pagination justify-content-center">
                      <li class="page-item" :class="{ disabled: currentPage === 1 }">
                        <router-link class="page-link page-link-prev" :to="getCategoryLink(the_category.id, the_category.name, currentPage - 1 )" aria-label="Previous" tabindex="-1" aria-disabled="true" @click="goToPreviousPage">
                      ...
                      <li class="page-item" v-for="page in generatePageLinks" :key="page" :class="{ active: page === currentPage }">
                        ...
                        <template v-if="isInteger(page)">
                          <router-link class="page-link" :to="getCategoryLink(the_category.id, the_category.name, page )" @click="goToThisPage(page)">
                            {{ page }}
                          </router-link>
                        </template>
                      ...
                      <li class="page-item" :class="{ disabled: currentPage === totalPages }">
                        <router-link class="page-link page-link-next" :to="getCategoryLink(the_category.id, the_category.name, currentPage + 1 )" aria-label="Next" @click="goToNextPage">
                          Next
               ...
               <aside class="col-lg-2 order-lg-first">
                  <div class="sidebar sidebar-shop sidebar-shop-category">
                     <div class="widget widget-clean">
                        <label>Filters:</label>
                        <button  @click="resetSortValues" class="btn btn-primary btn-sm sidebar-filter-clear">Clean All</button> 
                     </div>
                     ...
                     <div v-if="categories.length" class="widget widget-collapsible">
                        <h3 class="widget-title"> <a
                           data-toggle="collapse" href="#widget-1" role="button" aria-expanded="true" aria-controls="widget-1"
                           > Category
                           </a>
                        </h3>
                        <!-- End .widget-title -->
                           ...
                           <div class="widget-body">
                              <div class="filter-items filter-items-count">
                                 ...
                                 <div class="filter-item" v-for="category in categories" :key="category.id" >
                                    ...
                                    <!-- End .custom-checkbox --> <span class="item-count">{{ category.category_products_count }}</span>
                                 </div>
                     ...
                     <div class="widget widget-collapsible">
                        <h3 class="widget-title">
                           <a
                              data-toggle="collapse" href="#widget-4" role="button" aria-expanded="true" aria-controls="widget-4"
                              > Brand
                           </a> 
                        </h3>
                              ...
                              <div class="filter-items">
                                 <div v-for="brand in brands" :key="brand.id"  class="filter-item">
                                    <div class="custom-control custom-checkbox"> 
                                       ...
                                       @change="handleCheckboxBrandChange(brand.product_brand.id)"
                                       />
                                       <label class="custom-control-label" :for="'brand-' + brand.product_brand.id"
...
<script setup>
  ...
  import { useMetaKitchen } from "../../admin/composables/use-meta";
  ...
  const getCategoryLink = (id, name, page) => {
    ...
    if(name === undefined){
      return `/kitchen`;
       ...
       return `/kitchen/${id}/${transformedName}/page/${page}`;
  ...
  const title = ref('');
    ...
    // Call useMetaKitchen with the title
    useMetaKitchen({ title: title.value + "Food Service Equipment" });
