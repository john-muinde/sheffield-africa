...
export default function useProducts() {
    ...
    const storeProduct = async (product,  files) => {
        ...
        axios.post('/api/products', serializedPost, config)
            .then(response => {
    ...
    const updateProduct = async (product,  files) => {
        ...
        for (let item in product) {
          if (product.hasOwnProperty(item) && item !== 'categories') {
            ...
            if (value === null && (item === 'weight' || item === 'length' || item === 'height' || item === 'width' || item ===  'quantity' || item === 'cost_price' || item ===  'retail_price')) {
              value = 0;
            }
        ...
        await axios.post('/api/products/' + product.id, serializedPost, config)
            .then(response => {
    ...
    const deleteProductImage = async (id) => {
        swal({
            ...
            .then(result => {
                if (result.isConfirmed) {
                    axios.delete('/api/product-images/' + id)
                        .then(response => {
                            getProducts()
                            //router.push({name: 'products.index'})
                            router.go(0);
