...
export function useMediaDocuments(options = {}) {
  ...
  const processDocuments = async (fetchFunction, sorting = 'height') => {
    ...
    try {
      ...
      if (sorting === 'height') {
        documents.value = sortDocumentsByThumbnailHeight(processedDocuments);
      } else {
  ...
  const initializeDflip = (docs = documents.value) => {
    ...
    if (window.DFLIP?.parseBooks) {
      ...
      if (router.currentRoute.value.hash) {
        ...
        const brochure = docs.find((b) => router.currentRoute.value.hash.includes(b.slug));
        if (brochure) {
  ...
  const filterDocuments = (searchTerm = '') => {
    return documents.value.filter((doc) => {
      // Check filters
      const matchesFilters = Object.entries(filters).every(([key, value]) => doc[key] === value);
