<template>
    <TransitionRoot appear :show="true">
        <div class="w-full min-h-[400px] flex flex-col items-center justify-center p-6">
            <TransitionChild enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
                enter-to="opacity-100 scale-100" leave="duration-200 ease-in" leave-from="opacity-100 scale-100"
                leave-to="opacity-0 scale-95">
                <div class="text-center max-w-md mx-auto">
                    <!-- Loading State -->
                    <div v-if="type === 'loading'" class="animate-in fade-in duration-500">
                        <div class="relative mx-auto w-24 h-24 mb-6">
                            <slot name="loading-icon">
                                ...
                                <div class="absolute inset-3 rounded-full bg-primary-50 animate-pulse"></div>
                                <div class="absolute inset-[14px] rounded-full bg-primary-500"></div>
                            </slot>
                        </div>
                        ...
                        <p class="text-gray-500">
                            {{ loadingSubMessage || `Please wait while we fetch the ${contentType}` }}
                        </p>
                    </div>

                    <!-- Empty State -->
                    <div v-else-if="type === 'empty'" class="animate-in fade-in duration-500">
                        <div class="relative mx-auto w-24 h-24 mb-6">
                            <slot name="empty-icon">
                                ...
                                <div class="empty-state-animation">
                                    <svg class="w-full h-full text-gray-300" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor">
                                        <path v-if="contentIllustrations[contentType]"
                                            :d="contentIllustrations[contentType]" stroke-linecap="round"
                        ...
                        <p class="text-gray-500 mb-6">
                            {{ searchTerm
                                ...
                                : emptySubMessage || `${capitalize(contentType)} will appear here once added` }}
                        </p>
                        <div class="flex flex-wrap justify-center gap-3">
                            <button v-if="searchTerm" @click="$emit('clear-search')" class="btn-primary">
                    ...
                    <!-- Error State -->
                    <div v-else-if="type === 'error'" class="animate-in fade-in duration-500">
                        <div class="relative mx-auto w-24 h-24 mb-6">
                            <slot name="error-icon">
                                <!-- Default error animation -->
                                <div class="absolute inset-0 rounded-full bg-red-100 animate-pulse"></div>
                                <svg class="absolute inset-0 w-full h-full text-red-500 transform -rotate-12"
                        ...
                        <p class="text-gray-500 mb-6">
                            {{ errorSubMessage || `There was an error loading the ${contentType}` }}
                        </p>
                        <div class="flex flex-wrap justify-center gap-3">
                            <button @click="$emit('retry')" class="btn-error">
                                <svg class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <slot name="error-actions">
                                <button @click="$emit('clear-filters')" class="btn-secondary">
                                    <svg class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
